...
double timeToRun;
double verus_r=6.0; // Default value
...
//struct sockaddr_in adr_clnt2; /* add_clnt sufficient */
...
void* receiver_thread (void *arg)
{
    ...
    while (!terminate) {
        ...
        if (recvfrom(s, pdu, sizeof(udp_packet_t), 0, (struct sockaddr *)&adr_clnt, &len_inet) < 0) /* changed adr_clnt2 -> adr_clnt */
...
int main(int argc,char **argv) {
    ...
    while (i != (argc-1)) {
        ...
        if (!strcmp (argv[i], "-name")) {
            ...
            i=i+1;
            verus_r = std::stod(argv[i]);
        }
        else {
            std::cout << "syntax should be ./verus_server -name NAME -p PORT -t TIME (sec) -r PARAMETER (r)\n";
            exit(0);
        }
    }
    ...
    while (relativeTime <= timeToRun) {
        ...
        if (!slowStart) {
			...
			//fprintf(stderr,"R PARAM: %f\n",verus_r);
            if (dMaxLast/dMin > verus_r) {// VERUS_R) { /* removed for arg integration */
                if (!exitSlowStart) {
