...
//struct sockaddr_in adr_srvr2; /* only need one socket addr */
...
void TimeoutHandler( const boost::system::error_code& e) {
    ...
    z = sendto(s,"Hallo", strlen("Hallo"), 0, (struct sockaddr *)&adr_srvr, len_inet); /* changed adr_srvr2 -> adr_srvr */
    if ( z < 0 )
...
void* sending_thread (void *arg)
{
  ...
  while (!terminate) {
    ...
    if (sendingList.size() > 0 && (timestamp.tv_sec-pkt->seconds)*1000.0+(timestamp.tv_usec-pkt->millis)/1000.0 > delay) {
      // sending ACK
      z = sendto(s, pkt->pdu, sizeof(udp_packet_t), 0, (struct sockaddr *)&adr_srvr, len_inet); /* changed s1 -> 2, adr_srvr2 -> adr_srvr */
      free (pkt->pdu);
